/*
  Arduino IDE 1.8.13 версия прошивки oneWireSlave 0.0.3 бета от 03.11.21
...
void readOneWire(void) //эмуляция шины 1wire
{
  ...
  switch (oneWireRead()) { //читаем байт сетевого протокола
    case READ_ROM: //комманда отправить адрес
      for (uint8_t i = 0; i < sizeof(wireAddrBuf); i++) if (oneWireWrite(wireAddrBuf[i])) return; //отправка адреса на шину 1wire
      return; //выходим
    case MATCH_ROM: //комманда сравнить адрес
      for (uint8_t i = 0; i < sizeof(wireAddrBuf); i++) if (oneWireRead() != wireAddrBuf[i]) return; //чтение адреса с шины 1wire
      break; //продолжаем
